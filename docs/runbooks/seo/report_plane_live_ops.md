...
**Efect**: Worker-ul dă "abort", skip peste Priority, skip peste Trends. `overall_status: failed`.
**Motiv**:
...
**Acțiune**: 
- Ignori restul tab-urilor, problema e la sursă GSC. Aserviți cron-ul GSC să extragă manual un snapshot sau lasă-l pentru ziua următoare (Readiness Gate va impune `stale report` lock la ~48h).
