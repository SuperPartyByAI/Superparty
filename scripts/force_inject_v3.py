...
for filepath in astro_files:
    ...
    # If the marker is already there from a failed attempt, we need to strip it and everything after it 
    # up to </Layout> to ensure a clean slate.
    idx = content.find(MARKER)
    if idx != -1:
        # We strip the marker and everything up until the closing </Layout> tag
        layout_idx = content.find('</Layout>', idx)
        if layout_idx != -1:
            content = content[:idx] + content[layout_idx:]
        else:
            # If no layout tag found after marker, just strip everything after marker
            content = content[:idx]
...
print(f"Force injected clean util grid into {injected_count} Hub pages.")
