...
function style0(loc, d, charName) { // Stil: jurnal de calatorie + ghid practic
...
function style1(loc, d, charName) { // Stil: Q&A cu parinti tipici din zona
  ...
  `O altă întrebare frecventă de la părinții din ${loc}: „Vine cu personajul preferat al copilului meu?" Colecția SuperParty numără peste 50 de personaje actualizate constant. Personajul-favorite al copiilor din zona ${loc} și ${d.vecin} în acest sezon este ${charName}. Dar o condiție a serviciului nostru este că venim cu ORICE personaj ai rezervat — niciodată nu substituim un personaj cu altul fără acordul tău. Aceasta este una din clauzele explicite ale contractului de garanție.`,
...
function style2(loc, d, charName) { // Stil: reportaj etnografic + date concrete
...
function style4(loc, d, charName) { // Stil: articol comparativ + beneficii concrete
