...
# Find the start of _orig_seo_apply_task
start_idx = original_content.find('def _orig_seo_apply_task(site_id="superparty", apply_mode="report"):')

if start_idx == -1:
    print("Could not find _orig_seo_apply_task!")
    exit(1)

# Keep the content before this function
content_before = original_content[:start_idx]

enterprise_apply_code = """
def _orig_seo_apply_task(site_id="superparty", apply_mode="report"):
    \"\"\"Apply SEO plan. If apply_mode='real', safely inject content into Astro files with deterministic logic.\"\"\"
    import json
    import re
    import os
    import html
    ...
    from agent.common.git_pr import create_pr, git_commit_push_pr
    ...
    def get_deterministic_payload(clean_path):
        page_type = "unknown"
        location_label = ""
        ...
        # Use hash to deterministically pick FAQs
        ...
        payload["faqs"] = [base_pool[i] for i in order[:4]] # Pick exactly 4
    ...
    for opp in opportunities:
        ...
        clean_path = url_path.replace("https://www.superparty.ro", "").replace("https://superparty.ro", "").split("?")[0].strip("/")
        text = fpath.read_text(encoding="utf-8", errors="ignore")

        payload = get_deterministic_payload(clean_path)
